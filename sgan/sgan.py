...
class Discriminator(nn.Module):
    def __init__(self):
        ...
        self.aux_layer = nn.Sequential(
            nn.Linear(128 * ds_size ** 2, args.num_classes + 1),
            nn.Softmax(dim=1)
        )
...
for epoch in range(args.n_epochs):
    for i, (imgs, labels) in enumerate(dataloader):
        ...
        fake_aux_gt = LongTensor(batch_size).fill_(args.num_classes)
        ...
        # Loss for real images which is the supervised part
        real_pred, real_aux = discriminator(real_imgs)
        d_real_loss = (adversarial_loss(real_pred, valid) + auxiliary_loss(real_aux, labels)) / 2

        # Loss for fake images which is the unsupervised part
        ...
        print("[Epoch %d/%d] [Batch %d/%d] [D loss: %f, acc: %d%%] [G loss: %f]" % (
            epoch, args.n_epochs, i, len(dataloader), d_loss.item(), float(100 * d_acc), g_loss.item()))
